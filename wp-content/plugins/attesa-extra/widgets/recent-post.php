...
class AttesaExtraRecent extends WP_Widget {
	function __construct() {
		parent::__construct(
			'AttesaExtraRecent',
			__( 'Attesa Recent Posts with thumbnails', 'attesa-extra' ),
			array( 'classname' => 'AttesaExtraRecent', 'description' => __( 'Displays a list of your latest posts with thumbnails', 'attesa-extra' ) )
		);
	}
	private static function attesa_recentposts_defaults() {
		$defaults = array(
			'title' => __('Recent posts', 'attesa-extra'),
			'dis_posts' => '3',
			'cat_filter' => '',
		);
		return $defaults;
