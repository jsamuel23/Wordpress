...
require AE_PATH . '/panel/classes/importers/class-parsers.php';
	...
	function dispatch() {
		...
		switch ( $step ) {
			...
			case 2:
				...
				$this->id = (int) isset($_POST['import_id']);
...
<?php
	...
	function get_author_mapping() {
		...
		foreach ( (array) sanitize_text_field(wp_unslash($_POST['imported_authors'])) as $i => $old_login ) {
		...
		foreach ( $this->posts as $post ) {
			...
			*/
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				printf( esc_html__('%s &#8220;%s&#8221; already exists.', 'wordpress-importer'), esc_html($post_type_object->labels->singular_name), esc_html($post['post_title']) );
				echo '<br />';
	...
	function fetch_remote_file( $url, $post ) {
		...
		$headers = AWP_Demos_Helpers::wp_get_http( $url, $upload['file'] );
